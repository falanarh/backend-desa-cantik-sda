...
export const getSubMenu = async (id: string) => {
  return await SubMenu.findById(new mongoose.Types.ObjectId(id))
    .populate("subSubMenus") // Populate sub-submenus
...
export const updateSubMenu = async (
  ...
  return await SubMenu.findByIdAndUpdate(
    new mongoose.Types.ObjectId(id),
    data,
    { new: true }
  )
    .populate("subSubMenus")
    .populate("menu");
};
...
// Add SubSubMenu to a SubMenu
export const addSubSubMenuToSubMenu = async (
  subMenuId: string,
  subSubMenuId: string
) => {
  const subMenu = await SubMenu.findById(
    new mongoose.Types.ObjectId(subMenuId)
  );
  if (!subMenu) throw new Error("SubMenu not found");
  subMenu.subSubMenus = [
    ...(subMenu.subSubMenus || []),
    new mongoose.Types.ObjectId(subSubMenuId),
  ];
  return await subMenu.save();
};

// Remove SubSubMenu from a SubMenu
export const removeSubSubMenuFromSubMenu = async (
  ...
  if (!subMenu) throw new Error("SubMenu not found");
  subMenu.subSubMenus = subMenu.subSubMenus?.filter(
    (id) => id.toString() !== subSubMenuId
  );
  return await subMenu.save();
};
