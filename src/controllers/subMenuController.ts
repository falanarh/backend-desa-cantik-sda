...
export const addSubSubMenuToSubMenu = async (req: Request, res: Response) => {
    try {
        ...
        const subMenu = await SubMenuService.addSubSubMenuToSubMenu(req.params.id, subSubMenuId);
        res.json(subMenu);
    } catch (error : any) {
        res.status(500).json({ message: error.message });
    }
...
export const removeSubSubMenuFromSubMenu = async (req: Request, res: Response) => {
    try {
        const { subSubMenuId } = req.body;
        const subMenu = await SubMenuService.removeSubSubMenuFromSubMenu(req.params.id, subSubMenuId);
        res.json(subMenu);
    } catch (error : any) {
        res.status(500).json({ message: error.message });
    }
